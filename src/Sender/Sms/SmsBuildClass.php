...
class SmsBuildClass extends SmsDefineClass
{
    ...
    public function buildDlt($category, $key, $buildSmsData, $xmlDoc)
    {
        if ($this->setDlt()) {
    ...
    public function addDlt($buildSmsData, $category, $xmlDoc = null)
    {
        if ($this->isKeyPresent('DLT_TE_ID')) {
            $buildSmsData = $this->buildDlt($category, 'DLT_TE_ID', $buildSmsData, $xmlDoc);
        }
        return $buildSmsData;
    }
}
